...
class _SupplierListScreenState extends State<SupplierListScreen> {
  ...
  KeyEventResult _handleKeyEvent(FocusNode node, KeyEvent event) {
    if (event is! KeyDownEvent || _suppliers.isEmpty) {
    ...
    if (key == LogicalKeyboardKey.arrowDown) {
      setState(() {
        if (_highlightedIndex < 0) {
          ...
          _highlightedIndex = (_highlightedIndex + 1) % _suppliers.length;
        }
      });
    ...
    if (key == LogicalKeyboardKey.arrowUp) {
      setState(() {
        if (_highlightedIndex <= 0) {
          _highlightedIndex = _suppliers.length - 1;
        } else {
  ...
  void _onSearchChanged(String value) {
    ...
    _debounceTimer = Timer(const Duration(milliseconds: 400), () {
      ...
      // Filter is applied locally since the backend Supplier API doesn't support a search query param currently.
    });
  }
                        ...
                        appBar: AppBar(
                          ...
                          actions: [
                            IconButton(
                              ...
                              onPressed: _isLoading ? null : _fetchSuppliers,
                            ),
          ...
          Expanded(
            ...
            child: TextField(
              ...
              decoration: InputDecoration(
                hintText: 'Search by name, mobile, email, or city...',
  ...
  Widget _buildBodyContent({required bool isDesktop}) {
      ...
      return const Center(
        child: Column(
          ...
          children: [
            ...
            SizedBox(height: 16),
            Text('Loading suppliers from server...'),
          ],
        ),
      );
    }
    ...
    final displayList = _filteredSuppliers;
            ...
            children: [
              ...
              Text(
                _hasActiveFilters
                    ...
                    : 'Get started by creating your first supplier record',
                textAlign: TextAlign.center,
                style: TextStyle(color: Colors.grey.shade600),
              ),
              const SizedBox(height: 20),
  ...
  // Desktop Data Table
  Widget _buildDesktopDataTable(List<SupplierListItem> displayList) {
    final theme = Theme.of(context);
      ...
      children: [
        Container(
          ...
          child: Row(
            children: [
              Text(
                'Showing ${displayList.length} ${displayList.length == 1 ? 'supplier' : 'suppliers'}',
  ...
  // Mobile Card List
  Widget _buildMobileCardList(List<SupplierListItem> displayList) {
    final theme = Theme.of(context);
    final isDark = theme.brightness == Brightness.dark;
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        return Card(
          ...
          child: InkWell(
            borderRadius: BorderRadius.circular(12),
            onTap: () => _navigateToAddSupplier(supplier),
            child: Padding(
