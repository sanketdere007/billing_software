...
/// Supplier Master / Add / Edit screen
/// Saves supplier data via POST `/api/Supplier/InsertOrUpdateSupplier`
class SupplierMasterScreen extends StatefulWidget {
  final SupplierListItem? supplierToEdit;
  final int? supplierId;

  const SupplierMasterScreen({super.key, this.supplierToEdit, this.supplierId});
...
class _SupplierMasterScreenState extends State<SupplierMasterScreen> {
  ...
  final FocusNode _creditDaysFocusNode = FocusNode();
  ...
  bool get isEditing => _suppId > 0 || widget.supplierToEdit != null;
  ...
  void _initFormData() {
    if (widget.supplierToEdit != null) {
      _populateSupplierFields(widget.supplierToEdit!);
    } else if (widget.supplierId != null && widget.supplierId! > 0) {
      _fetchSupplierDetails(widget.supplierId!);
    } else {
      _isActive = true;
  ...
  Future<void> _fetchSupplierDetails(int suppId) async {
    setState(() {
      _isFetchingDetails = true;
    });

    try {
      final List<SupplierListItem> suppliers = await _supplierService.getAllSuppliers(suppId: suppId);
      if (suppliers.isNotEmpty && mounted) {
        _populateSupplierFields(suppliers.first);
      }
    } catch (e) {
      if (mounted) {
        await showErrorDialog(context, 'Failed to load supplier details: $e');
      }
    } finally {
      if (mounted) {
        setState(() {
  ...
  void _populateSupplierFields(SupplierListItem supplier) {
    ...
    _gstController.text = supplier.suppGSTNo;
    _panController.text = supplier.suppPANNo;
    _creditLimitController.text = supplier.suppCreditLimit > 0 ? supplier.suppCreditLimit.toStringAsFixed(2) : '';
    _creditDaysController.text = supplier.suppCreditDays > 0 ? supplier.suppCreditDays.toString() : '';
    _addressController.text = supplier.suppAddress;
    _areaController.text = supplier.suppAreaName ?? '';
    _pincodeController.text = supplier.suppPincode;
    _countryController.text = supplier.suppCountry.isNotEmpty ? supplier.suppCountry : 'India';
    _isActive = supplier.suppIsActive;
    _selectedStateName = supplier.suppStateName ?? '';
    _selectedCityName = supplier.suppCityName ?? '';
    _selectedAreaName = supplier.suppAreaName ?? '';

    _selectedStateId = supplier.suppStateId > 0 ? supplier.suppStateId : null;
    _selectedCityId = supplier.suppCityId > 0 ? supplier.suppCityId : null;
    _selectedAreaId = supplier.suppAreaId > 0 ? supplier.suppAreaId : null;

    // Resolve state ID if available
    if (_selectedStateId == null && _selectedStateName.isNotEmpty) {
      final matchedState = _stateService.getStateByName(_selectedStateName);
    ...
    if (_selectedCityId == null && _selectedCityName.isNotEmpty) {
      final matchedCity = _cityService.getCityByName(_selectedCityName);
      if (matchedCity != null) {
    ...
    if (_selectedAreaId == null && _selectedAreaName.isNotEmpty) {
      try {
        final matchedArea = _areaService.areas.firstWhere(
          (a) => a.areaName.trim().toLowerCase() == _selectedAreaName.trim().toLowerCase(),
        );
  ...
  Future<void> _saveSupplier({bool saveAndNew = false}) async {
    ...
    try {
      ...
      if (areaId == 0 && (_selectedAreaName.isNotEmpty || _areaController.text.trim().isNotEmpty)) {
        final aName = _selectedAreaName.isNotEmpty ? _selectedAreaName : _areaController.text.trim();
        try {
      ...
      final request = SupplierUpsertRequest(
        ...
        suppAlternateMobileNo: _altMobileController.text.trim(),
        suppEmail: _emailController.text.trim(),
        suppGSTNo: _gstController.text.trim().toUpperCase(),
        suppPANNo: _panController.text.trim().toUpperCase(),
        suppAddress: _addressController.text.trim(),
        suppAreaId: areaId,
        suppCityId: cityId,
        suppStateId: stateId,
        suppPincode: _pincodeController.text.trim(),
        suppCountry: _countryController.text.trim().isNotEmpty ? _countryController.text.trim() : 'India',
        suppPaymentTerms: '',
        suppCreditLimit: double.tryParse(_creditLimitController.text.trim()) ?? 0.0,
        suppCreditDays: int.tryParse(_creditDaysController.text.trim()) ?? 0,
        suppBranchId: (isEditing && widget.supplierToEdit != null && widget.supplierToEdit!.suppBranchId > 0)
            ? widget.supplierToEdit!.suppBranchId
            : ((sessionService.selectedBranchId != null && sessionService.selectedBranchId! > 0) ? sessionService.selectedBranchId! : 1),
        suppCompId: (isEditing && widget.supplierToEdit != null && widget.supplierToEdit!.suppCompId > 0)
            ? widget.supplierToEdit!.suppCompId
            : ((sessionService.selectedCompId != null && sessionService.selectedCompId! > 0) ? sessionService.selectedCompId! : 1),
        suppIsActive: _isActive,
        suppCreatedBy: isEditing ? 0 : _currentEmpId,
        suppModifiedBy: _currentEmpId,
      );

      final response = await _supplierService.insertOrUpdateSupplier(request);
  ...
  Widget build(BuildContext context) {
    return SaveClearShortcuts(
      onSave: () {
        if (!_isLoading) {
          _saveSupplier(saveAndNew: false);
        }
      },
      onClear: () {
        if (!isEditing) {
  ...
  Widget _buildFormCard(BuildContext context, bool isDesktop) {
    ...
    return Form(
      ...
      child: Column(
        ...
        children: [
          // Section 1: Supplier & Business Info
          _buildCardSection(
            context,
                ...
                if (isDesktop) ...[
                  ...
                  Row(
                    ...
                    children: [
                      ...
                      const SizedBox(width: 16),
                      Expanded(child: _buildCreditDaysField()),
                    ],
                  )
                ] else ...[
                  _buildGstField(),
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                          _isActive
                              ? 'Active - Can be selected in purchase bills.'
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    if (!isEditing) ...[
                      ...
                      OutlinedButton.icon(
                        onPressed: _isLoading ? null : () => _saveSupplier(saveAndNew: true),
                    ...
                    const SizedBox(width: 12),
                    FilledButton.icon(
                      onPressed: _isLoading ? null : () => _saveSupplier(saveAndNew: false),
                      icon: _isLoading
                          ? const SizedBox(
                              width: 18,
                              height: 18,
  ...
  Widget _buildCreditLimitField() {
    return TextFormField(
      ...
      textInputAction: TextInputAction.next,
      onFieldSubmitted: (_) => _creditDaysFocusNode.requestFocus(),
      decoration: InputDecoration(
  ...
  Widget _buildAreaField() {
    return AreaDropdown(
      ...
      selectedAreaName: _selectedAreaName.isNotEmpty ? _selectedAreaName : _areaController.text,
      ...
      onChanged: (AreaListItem? area) {
        setState(() {
          ...
          if (area != null && _selectedCityId == null && area.cityId != null && area.cityId! > 0) {
            _selectedCityId = area.cityId;
            _selectedCityName = area.cityName;
          }
          if (area != null && _selectedStateId == null && area.stateId != null && area.stateId! > 0) {
