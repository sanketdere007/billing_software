...
import 'supplier_pending_invoice_report_screen.dart';

class SupplierOutstandingReportScreen extends StatefulWidget {
  const SupplierOutstandingReportScreen({super.key});

  @override
  State<SupplierOutstandingReportScreen> createState() => _SupplierOutstandingReportScreenState();
}

class _SupplierOutstandingReportScreenState extends State<SupplierOutstandingReportScreen> {
  ...
  List<SupplierOutstandingReportItem> _reportData = [];
  ...
  Future<void> _fetchMoreData() async {
    ...
    try {
      ...
      final data = await _supplierService.getSupplierOutstandingReport(
        pageNumber: nextPage,
        pageSize: _pageSize,
        search: _searchQuery,
      );
  ...
  Future<List<SupplierOutstandingReportItem>> _collectAllReportData() async {
    final allRecords = List<SupplierOutstandingReportItem>.from(_reportData);
    ...
    try {
      ...
      if (records.isEmpty) {
        await showWarningDialog(
          context,
          'No supplier outstanding records available to export.',
        );
        return;
      }

      final result = await ReportExcelExportService.exportSupplierOutstanding(records);
      if (!mounted) return;
      ...
      if (!mounted) return;
      await showErrorDialog(context, 'Failed to export supplier outstanding report: $e');
    } finally {
      if (mounted) {
        setState(() {
          _isExporting = false;
  ...
  void _navigateToPendingInvoice(SupplierOutstandingReportItem item) {
    Navigator.of(context).push(
      MaterialPageRoute(
        builder: (context) => SupplierPendingInvoiceReportScreen(
          supplierId: item.suppId,
          supplierName: item.suppName,
        ),
      ),
    ).then((_) {
      // Re-focus after coming back
      _screenFocusNode.requestFocus();
    });
  }
                ...
                body: Row(
                  children: [
                    ...
                    Expanded(
                      child: Scaffold(
                        appBar: AppBar(
                          title: const Text('Supplier Outstanding Report'),
                          actions: [
                            Container(
                              width: 250,
                ...
                Container(
                  ...
                  child: const Row(
                    children: [
                      ...
                      Expanded(flex: 3, child: Text('Supplier Name', style: TextStyle(fontWeight: FontWeight.bold))),
                      ...
                      Expanded(flex: 2, child: Text('Purchase Amt', style: TextStyle(fontWeight: FontWeight.bold))),
                    ...
                    itemBuilder: (context, index) {
                      ...
                      return InkWell(
                        onTap: () {
                          ...
                          _navigateToPendingInvoice(item);
                        },
                        child: Container(
                          height: 53,
                          padding: const EdgeInsets.symmetric(horizontal: 20),
                          ...
                          child: Row(
                            children: [
                              ...
                              Expanded(flex: 3, child: Text(item.suppName)),
                              Expanded(flex: 2, child: Text(item.suppMobileNo)),
                              Expanded(flex: 2, child: Text(item.suppCityName)),
                              Expanded(flex: 2, child: Text(item.totalPurchaseAmount.toStringAsFixed(2))),
                              ...
                              Expanded(
                                ...
                                child: Text(
                                  ...
                                  style: TextStyle(
                                    ...
                                    color: item.outstandingAmount > 0 ? Colors.red.shade700 : Colors.green.shade700,
                                  ),
                                ),
                              ),
                            ],
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        return Card(
          ...
          child: ListTile(
            title: Text(item.suppName, style: const TextStyle(fontWeight: FontWeight.bold)),
            subtitle: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('Mobile: ${item.suppMobileNo} | City: ${item.suppCityName}'),
                Text(
                  'Outstanding: ${item.outstandingAmount.toStringAsFixed(2)}',
                  style: TextStyle(
                    fontWeight: FontWeight.bold,
                    color: item.outstandingAmount > 0 ? Colors.red.shade700 : Colors.green.shade700,
                  ),
...
class _SupplierSearchDelegate extends SearchDelegate<String> {
  ...
  _SupplierSearchDelegate({required this.onSearch});
