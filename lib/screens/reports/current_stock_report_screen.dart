...
class CurrentStockReportScreen extends StatefulWidget {
  ...
  @override
  State<CurrentStockReportScreen> createState() => _CurrentStockReportScreenState();
}
...
class _CurrentStockReportScreenState extends State<CurrentStockReportScreen> {
  ...
  Future<void> _fetchMoreData() async {
    ...
    try {
      ...
      final response = await currentStockService.getCurrentStock(
        search: _searchQuery,
        pageNumber: nextPage,
        pageSize: _pageSize,
      );
  ...
  Future<List<CurrentStock>> _collectAllReportData() async {
    final allRecords = List<CurrentStock>.from(_reportData);
    ...
    try {
      ...
      if (records.isEmpty) {
        await showWarningDialog(
          context,
          'No current stock records available to export.',
        );
        return;
      }

      final result = await ReportExcelExportService.exportCurrentStock(records);
      if (!mounted) return;
      ...
      if (!mounted) return;
      await showErrorDialog(context, 'Failed to export current stock report: $e');
    } finally {
      if (mounted) {
        setState(() {
          _isExporting = false;
                ...
                body: Row(
                  children: [
                    ...
                    Expanded(
                      child: Scaffold(
                        appBar: AppBar(
                          title: const Text('Current Stock Report'),
                          actions: [
                            Container(
                              width: 200,
  ...
  Widget _buildBodyContent({required bool isDesktop}) {
    return Column(
      children: [
        if (!isDesktop) _buildMobileSearch(),
        Expanded(
            ...
            Container(
              ...
              child: const Row(
                children: [
                  ...
                  Expanded(flex: 2, child: Text('Brand', style: TextStyle(fontWeight: FontWeight.bold))),
                  ...
                  Expanded(flex: 1, child: Text('Unit Value', style: TextStyle(fontWeight: FontWeight.bold))),
                  Expanded(flex: 2, child: Text('Available Stock', style: TextStyle(fontWeight: FontWeight.bold))),
                ],
              ),
            ),
            // List
                    ...
                    child: Container(
                      ...
                      child: Row(
                        children: [
                          ...
                          Expanded(flex: 2, child: Text(item.brandName.isNotEmpty ? item.brandName : 'N/A')),
                          Expanded(flex: 2, child: Text(item.categoryName.isNotEmpty ? item.categoryName : 'N/A')),
                          Expanded(flex: 1, child: Text(item.unitShortName.isNotEmpty ? item.unitShortName : 'N/A')),
                          Expanded(flex: 1, child: Text(item.unitValue.toStringAsFixed(2))),
                          Expanded(
                            flex: 2, 
                            child: Text(
                              item.availableStock.toStringAsFixed(2),
                              style: TextStyle(
                                fontWeight: FontWeight.bold,
                                color: item.availableStock > 0 ? Colors.green.shade700 : Colors.red.shade700,
                              ),
                            ),
                          ),
                        ],
        ...
        return Card(
          ...
          child: ListTile(
            ...
            subtitle: Column(
              ...
              children: [
                Text('Brand: ${item.brandName.isNotEmpty ? item.brandName : 'N/A'} • Category: ${item.categoryName.isNotEmpty ? item.categoryName : 'N/A'}'),
                Text('Unit: ${item.unitShortName.isNotEmpty ? item.unitShortName : 'N/A'} • Value: ${item.unitValue.toStringAsFixed(2)}'),
                Text(
                  'Available Stock: ${item.availableStock.toStringAsFixed(2)}',
                  style: TextStyle(
                    fontWeight: FontWeight.bold,
                    color: item.availableStock > 0 ? Colors.green.shade700 : Colors.red.shade700,
                  ),
