...
/// Category List Screen with search, status filter, keyboard navigation, and navigation to CategoryMasterScreen
...
class _CategoryListScreenState extends State<CategoryListScreen> {
  ...
  KeyEventResult _handleKeyEvent(FocusNode node, KeyEvent event) {
    if (event is! KeyDownEvent || _categories.isEmpty) {
    ...
    if (key == LogicalKeyboardKey.arrowDown) {
      setState(() {
        if (_highlightedIndex < 0) {
          ...
          _highlightedIndex = (_highlightedIndex + 1) % _categories.length;
        }
      });
    ...
    if (key == LogicalKeyboardKey.arrowUp) {
      setState(() {
        if (_highlightedIndex <= 0) {
          _highlightedIndex = _categories.length - 1;
        } else {
    ...
    if (key == LogicalKeyboardKey.enter ||
      ...
      if (_highlightedIndex >= 0 && _highlightedIndex < _categories.length) {
        _navigateToAddCategory(_categories[_highlightedIndex]);
        return KeyEventResult.handled;
      }
    }
  ...
  Future<void> _fetchCategories() async {
    ...
    try {
      await _categoryService.getAllCategories(isActive: _selectedStatus);
  ...
  void _applyFilters() {
    ...
    final allCategories = _categoryService.categories;
    ...
    setState(() {
      if (query.isEmpty) {
        ...
        _categories = allCategories.where((c) {
          return c.catName.toLowerCase().contains(query);
        }).toList();
      }

      _isLoading = false;
      if (_categories.isNotEmpty) {
        _highlightedIndex = _highlightedIndex.clamp(0, _categories.length - 1);
      } else {
        _highlightedIndex = 0;
      }
    });
  ...
  void _navigateToAddCategory([CategoryListItem? category]) async {
    final result = await Navigator.of(context).push<bool>(
      MaterialPageRoute(
        builder: (context) => CategoryMasterScreen(categoryToEdit: category),
      ),
    );
      ...
      return const Center(
        child: Column(
          ...
          children: [
            ...
            SizedBox(height: 16),
            Text('Loading categories from server...'),
          ],
        ),
      );
    }
            ...
            children: [
              ...
              Text(
                _hasActiveFilters
                    ...
                    : 'Get started by creating your first category master record',
                textAlign: TextAlign.center,
                style: TextStyle(color: Colors.grey.shade600),
              ),
              const SizedBox(height: 20),
      ...
      children: [
        ...
        Container(
          ...
          child: Row(
            children: [
              Text(
                'Showing ${_categories.length} ${_categories.length == 1 ? 'category' : 'categories'}',
                  ...
                  Column(
                    ...
                    children: [
                      Container(
                        ...
                        child: Text(
                          category.catIsActive ? 'Active' : 'Inactive',
                          style: TextStyle(
                            color: category.catIsActive
