...
class CategoryDropdown extends StatefulWidget {
  ...
  final ValueChanged<CategoryListItem?>? onChanged;
  final String? Function(CategoryListItem?)? validator;
...
class _CategoryDropdownState extends State<CategoryDropdown> {
  ...
  String? _error;
  CategoryListItem? _selectedItem;
  late FocusNode _focusNode;
  bool _isFocused = false;
  List<CategoryListItem> _items = [];
  ...
  void _syncSelected() {
    ...
    try {
      _selectedItem = _items.firstWhere((e) => e.catId == widget.selectedCategoryId);
    } catch (_) {
      _selectedItem = CategoryListItem(
        catId: widget.selectedCategoryId!,
        catName: 'Category #${widget.selectedCategoryId}',
        catDescription: '',
        catCreatedBy: 0,
        catModifiedBy: 0,
      );
  ...
  Future<void> _loadItems() async {
    ...
    try {
      final items = await _service.getAllCategories(isActive: true);
      if (mounted) {
        _items = items;
        _syncSelected();
      }
  ...
  void _openSearchDialog([FormFieldState<CategoryListItem?>? fieldState]) async {
    if (!widget.enabled || _isLoading) return;
    if (_items.isEmpty) await _loadItems();
    if (!mounted) return;

    final picked = await showDialog<CategoryListItem?>(
      context: context,
      builder: (context) => _SearchDialog(items: _items, selectedId: _selectedItem?.catId),
    );
  ...
  KeyEventResult _handleKeyEvent(FocusNode node, KeyEvent event, FormFieldState<CategoryListItem?>? fieldState) {
  ...
  Widget build(BuildContext context) {
    ...
    if (_isLoading) {
      return Container(
        ...
        child: Row(children: [const Icon(Icons.category_outlined, size: 20, color: Colors.grey), const SizedBox(width: 10), Expanded(child: Text('Loading...', style: TextStyle(color: theme.colorScheme.onSurfaceVariant))), const SizedBox(width: 16, height: 16, child: CircularProgressIndicator(strokeWidth: 2))]),
      );
    }
    ...
    return FormField<CategoryListItem?>(
      initialValue: _selectedItem,
      validator: (val) {
        if (widget.validator != null) return widget.validator!(_selectedItem);
        if (widget.isRequired && _selectedItem == null) return 'Please select a category';
        return null;
      },
      builder: (fieldState) {
            ...
            Focus(
              ...
              child: InkWell(
                ...
                child: Container(
                  ...
                  child: Row(
                    children: [
                      Icon(Icons.category_outlined, size: 20, color: _isFocused ? theme.colorScheme.primary : theme.colorScheme.onSurfaceVariant),
                      ...
                      Expanded(
                        child: Column(
                          ...
                          children: [
                            ...
                            Text(_selectedItem?.catName ?? widget.hintText, style: TextStyle(color: _selectedItem == null ? theme.colorScheme.onSurfaceVariant : theme.colorScheme.onSurface), maxLines: 1, overflow: TextOverflow.ellipsis),
                          ],
                        ),
                      ),
...
class _SearchDialogState extends State<_SearchDialog> {
  ...
  List<CategoryListItem> _filtered = [];
  ...
  void initState() {
    ...
    if (widget.selectedId != null) {
      final idx = widget.items.indexWhere((e) => e.catId == widget.selectedId);
  ...
  void _onSearch() {
    ...
    setState(() {
      _filtered = query.isEmpty ? widget.items : widget.items.where((e) => e.catName.toLowerCase().contains(query)).toList();
      _highlightedIndex = 0;
    });
    _scrollToIndex(0);
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Dialog(
      ...
      child: ConstrainedBox(
        ...
        child: Column(
          children: [
            Padding(padding: const EdgeInsets.all(16), child: TextField(controller: _searchCtrl, focusNode: _searchFocus, decoration: InputDecoration(hintText: 'Search category...', prefixIcon: const Icon(Icons.search), border: OutlineInputBorder(borderRadius: BorderRadius.circular(8)), filled: true))),
            Expanded(
              child: _filtered.isEmpty ? const Center(child: Text('No categories found')) : ListView.builder(
                controller: _scrollCtrl,
                itemCount: _filtered.length,
                itemBuilder: (context, index) {
                  final item = _filtered[index];
                  final isSel = item.catId == widget.selectedId;
                  final isHigh = index == _highlightedIndex;
                  return Container(
                    height: 56,
                    color: isHigh ? theme.colorScheme.primary.withOpacity(0.2) : Colors.transparent,
                    child: ListTile(
                      title: Text(item.catName, style: TextStyle(color: isSel || isHigh ? theme.colorScheme.primary : null, fontWeight: isSel || isHigh ? FontWeight.bold : FontWeight.normal)),
