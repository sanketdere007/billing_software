  ...
  void _openSearchDialog([FormFieldState<CategoryListItem?>? fieldState]) async {
    ...
    final picked = await showDialog<CategoryListItem?>(
      ...
      builder: (context) => _SearchDialog(
        items: _items,
        selectedId: _selectedItem?.catId,
        isFilter: widget.isFilter,
        allOptionLabel: widget.allOptionLabel ?? 'All Categories',
      ),
    );

    if (picked != null) {
      final effectiveItem = (picked.catId == -1 || picked.catName == '__ALL_CATEGORIES__') ? null : picked;
    ...
    return FormField<CategoryListItem?>(
      ...
      builder: (fieldState) {
        ...
        final displayText = _selectedItem != null
            ? _selectedItem!.catName
            : (widget.isFilter ? (widget.allOptionLabel ?? 'All Categories') : widget.hintText);
                  ...
                  child: Row(
                    children: [
                      if (widget.prefixIcon != null) ...[
                        ...
                        Icon(Icons.category_outlined, size: 18, color: _isFocused ? theme.colorScheme.primary : (widget.enabled ? theme.colorScheme.primary : theme.colorScheme.onSurfaceVariant.withOpacity(0.5))),
                        const SizedBox(width: 8),
                      ],
                      Expanded(
...
class _SearchDialog extends StatefulWidget {
  static CategoryListItem allOption = CategoryListItem(
    catId: -1,
    catName: '__ALL_CATEGORIES__',
    catDescription: '',
    catCreatedBy: 0,
    catModifiedBy: 0,
  );
      ...
      child: ConstrainedBox(
        ...
        child: Column(
          children: [
            ...
            Expanded(
              child: _totalItems == 0 ? const Center(child: Text('No categories found')) : ListView.builder(
